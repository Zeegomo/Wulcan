...
public class Test {
	...
	static double[][] matrix = {
			{0.9999	,0.0100495	,-0.0099495},
			{-0.0099495	,0.9999	,0.0100495},
			{0.0100495	,-0.0099495	,0.9999},
			};
	...
	public static void main(String[] args) {
		...
		while(view.isAvailable()) {
			for (int i = 0; i < 8; i++) {
				...
				points_p[i].x = points_p[i].x / (Math.tan(fov/2)*(points[i].z + 6));
				points_p[i].y = points_p[i].y / (Math.tan(fov/2)*(points[i].z + 6));
	...
	static void rotate(Point3D p) {
		Point3D t = new Point3D(p.x, p.y, p.z);
		p.x = t.x * matrix[0][0] + t.y * matrix[0][1] + t.z * matrix[0][2];
		p.y = t.x * matrix[1][0] + t.y * matrix[1][1] + t.z * matrix[1][2];
		p.z = t.x * matrix[2][0] + t.y * matrix[2][1] + t.z * matrix[2][2];
